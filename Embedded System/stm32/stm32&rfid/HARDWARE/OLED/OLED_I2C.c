...
void OLED_ShowNum(unsigned char x, unsigned char y, u32 num,unsigned char len, unsigned char TextSize,u8 mode)
{
	u8 t,temp;
	u8 enshow=0;			
		switch(TextSize)
		{
			case 1:for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode==1)
				{
				OLED_ShowChar(x+6*t,y,' ',1);
				}
				else OLED_ShowChar(x+6*t,y,'0',1);
				continue;
			}else enshow=1; 
		 	 
		}
	 	OLED_ShowChar(x+6*t,y,temp+'0',1); 
	}
	break;
			case 2:
		for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				if(mode==1)
				{
				OLED_ShowChar(x+8*t,y,' ',2);
				}
				else OLED_ShowChar(x+8*t,y,'0',2);
				continue;
			}else enshow=1; 
		 	 
		}
	 	OLED_ShowChar(x+8*t,y,temp+'0',2); 
	}break;
	}
}
//m^n函数
